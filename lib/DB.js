...
class DB {
            ...
            if (u) {
                ...
                Category.find({
                    ...
                    if (cats) {
                        cats.map(cat => {
                            if (cat) {
                                console.log("Category : " + cat + " interested: " + cat.interested);
    ...
    getThreads(ids, callback) {
        console.log("Ids: " + typeof (ids));
        // let ds = ids.map(id => parseInt(id));
        Thread.find({
            id: {
                $in: ids
            }
    ...
    getReplies(type, id, callback) {
        if (type == "thread") {
            ...
            User.findOne({
                ...
                let filtered = user.ratings.filter((item) => {
                    return (item.type == 'thread');
                });
                let starredIDs = filtered.map((thread) => {
                    return thread.id
                });
        ...
        if (type === 'thread') {
            ...
            Thread.findOne({
                ...
                if (thread) {
                    ...
                    let n = thread.numRatings;
                    let newRating = (n * thread.rating + rating) / (n + 1);
            ...
            Reply.findOne({
                ...
                if (reply) {
                    let n = reply.numRatings;
                    let newRating = (n * reply.rating + rating) / (n + 1);
    ...
    bookmark(userName, threadID) {
        User.findOne({
            ...
            if (user) {
                ...
                console.log("OLOL : " + user.bookmarks.length);
    ...
    // Store Predicions
    storePredictions(preds) {}

    getUserInterests(uName, callback) {
        User.findOne({
            userName: uName
        }).exec((err, user) => {
            callback({
